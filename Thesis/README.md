...
-   Finally we consider total return indexes because it measure the true
    returns to investors. It factors changes in dividend yield and
    prices in the index.
...
-   to get a more nuanced picture of the performance over time, we
    stratify but first.

-   lets figure out a systematic way to stratifying? For this I do not
    have a way but I looked at the rolling standard deviation and by
    eyeballing it saw that meaningful data points are concentrated in
    the 95th and 5 percentile.
...
```
    ...
    ##  4 EU_DG  Low Vol Period   2217                  -0.612
    ##  5 EU_HY  High Vol          224                   7.22 
    ##  6 EU_HY  Low Vol Period   2217                   0.307
...
-   we notice that there have been varying lengths in vol periods
    between geographies.
-   may lead suspicious results in the sense that there may not be
    enough data points to string good representation of returns
...
So typically what characterizes a high vol period and how does that
influence our value stock. what main economoic indicators give the most
impact in affecting returns?
...
but their strategies are evenly split given our sample.
...
analysis here should take a different form. lets do this by sections. -
HY indexes at the start had positive IR. Expect for SA and UK - Barring
SA from pre 2010 there have moved into negative. - DG indexes start
negative expect for the US. Apart from the slight postive in recent
times from the US all are negative. - For regions with DG and HY
portfolios, the consistency of returns tends to mirror each other. Given
their characterization, their drivers should broadly be similar. -
Comparing amongst regions, visually US and SA HY as off late have
mirroring effects, and this term is used very loosely.There has been a
general co movement in IR for the different JP EM. - SA DG, SA HY and JP
DG have had similar movements pre 2010 and more recently an upward
trend.

Possible analysis could be to regress the information ratios on a set of
economic variables. Maybe do a rolling R2 of variables that may have an
effect on the consistency of returns.
...
markets and south africa show the least variation.
...
portfolios. tratification, all portfolios perform as expected
