...
excess_return_df <- list( UK,US ,UK, JP, SA, EM) %>%
  reduce(inner_join, by='date') %>% gather(Regions, Cumulative_Excess_Return, -date)

cumulative_excess_return_df <- list( UK,US ,UK, JP, SA, EM) %>%
  reduce(inner_join, by='date') %>% gather(Regions, Cumulative_Excess_Return, -date) %>% 
  mutate(Year = as.numeric(format(date , "%Y"))) %>% 
  group_by( Regions) %>% 
  summarise(Cumulative_Excess_Return = last(Cumulative_Excess_Return), Median = median(Cumulative_Excess_Return) ) %>% distinct()
...
# # US stat df
 ...
 A <- US_df %>% filter(
   Date %in% hivol_per_vector_us) %>%  MY_excess_return(., "US_HY", "US")
 B <- US_df %>% filter(
   Date %in% lovol_per_vector_us) %>%  MY_excess_return(., "US_HY", "US")
 
 US <- left_join(A, B, "date")  %>% filter(date >= fmxdat::safe_month_min(last(date), N = 36)) %>%
   group_by(ticker) %>% 
