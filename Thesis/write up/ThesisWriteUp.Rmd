...
# Globally traded Dividend Portfolios
...
all_excess_returns_df  <- list(A, B, C, D, E, F1, G,H,J, K, L) %>%
  reduce(inner_join, by='date') %>% gather(Ticker, excess, -date)

Table <- all_excess_returns_df %>%  
  group_by(Ticker) %>% 
  summarise(Cumulative_mean_return = mean(excess), Median_cumulative_return = median(excess))

data = Table %>% tibble::as_tibble()
  addtorow          <- list()
  addtorow$pos      <- list()
  addtorow$pos[[1]] <- c(0)
...
table <- xtable(data, caption = "Aggregate Dividend Effects")
  print.xtable(table,
...
library(xtable)
table <- xtable(divi_vol_df, caption = "Short Table Example \\label{tab1}")
  print.xtable(table,
             # tabular.environment = "longtable",
             floating = TRUE,
             table.placement = 'H',
...
g <- ggplot(plotdf, aes(date, RollRets, color = ticker)) +
  ...
  labs(x = "", y = "Cumulative  Returns", caption = "Data:\n Bloomberg ",
       title = "Global Dividend Portfolio Rolling 3 Year Returns", 
       legend = T) 
