...
CB_df <- CB_df %>% rename(Quarters = "Months") %>% arrange(desc("Annualized Return"))
...
```{r}
US_rates <- rate_data %>% select(Date, US) %>% filter(!is.na(US))
```
...
hivol_per_vector_uk %>% 
  data.frame() %>%
  setNames(c("Date")) %>% arrange(Date) %>% mutate(Year = format(Date , "%Y")) %>% 
  group_by(Year) %>% filter(Date == first(Date)| Date == last(Date))

data <- UK_rates %>% filter(!is.na(UK))

shade <- data.frame(
  x1 = as.POSIXct("2010-05-04"),
  x2 = as.POSIXct("2013-05-05"),
  y1 = -Inf, 
  y2 = Inf   
)

data %>% ggplot() +
  geom_line(aes(Date, UK)) + 
  labs(x = "", y = "", title = "", subtitle = "", caption = "Source: Bloomberg and authors' own calculations") +
...
```{r}
JP_rates <- rate_data %>% select(Date, JP) %>%  filter(!is.na(JP))
```


```{r}
SA_rates <- rate_data %>% select(Date, SA) %>%  filter(!is.na(SA))
```


```{r}
ECB_rates <- rate_data %>% select(Date, ECB) %>%  filter(!is.na(US))
